...
async def test_a_mode_tightened_during_the_pass_aborts_before_any_durable_write(
    ...
    overlapping the user's next message). At the write boundary the mode is
    re-resolved: the pass aborts with the same SEL denial and memo entry as a
    refusal before the snapshot, the store receives no write and the offset does
    not advance. All three entry points cross the same boundary because all
    three run ``_consolidate``. Mutation: drop the boundary re-check -- the store
