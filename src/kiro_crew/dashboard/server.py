...
def _register_crewmate_prune_gate(app: web.Application, state: DashboardState) -> None:
    ...
    alone and readiness is not gated by it. Reads are never held: the gate
...
def _kick_crewmate_prune(state: DashboardState) -> None:
    ...
    set, which happens in ``finally`` whatever the pass does; ``start_dashboard``
    awaits the same event before the slot restores so a removed crewmate's DM
    slot is not rebuilt. On every boot but the first after the upgrade the pass
    is one marker stat.
    """
...
async def start_dashboard(
    ...
    # with the session count, so it stays off the boot path -- while the gate
    # armed before the bind holds every mutating request until it settles. The
    # slot restores below await the same event.
    ...
    # The crewmate prune must have settled before a DM slot is rebuilt: a slot
    # pointing at a row the pass removes would otherwise come back. Bounded by
    # the same budget the request gate uses; the pass sets the event in
    # ``finally`` whatever it does, so this only trips on a wedged worker, and
    # then restoring late beats never restoring.
    try:
        await asyncio.wait_for(
            state.crewmate_prune_settled.wait(), timeout=_CREWMATE_PRUNE_GATE_TIMEOUT_S
        )
    except asyncio.TimeoutError:
        logger.warning("crewmate prune has not settled; restoring sessions without it")
