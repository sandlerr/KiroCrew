...
unknown (the service shields its persist), so the entry is left as written and
an owner entry over a stopped loop is resumable only by the owner. OFF on an
...
(strict, the record kept for its words), and a CANCELLED initial arm first
waits for the service's shielded `add` to settle (re-shielded across repeat
